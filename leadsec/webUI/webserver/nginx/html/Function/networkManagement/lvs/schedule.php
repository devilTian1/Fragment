<?php
    ...
    function getAddCmd() {
        ...
        for ($i=1 ; $i<17 ; $i++) {
            ...
            if(!empty($realip) && !empty($realport)) {
                $cmdBuf[] = 'lvsctl add service vip ' . $virtualAddr . ' vport ' . $virtualPort . ' rip ' . $realip . ' rport ' . $realport;
            }
        }
        return $cmdBuf;
    }
    ...
    if ($_POST['type'] === 'add') {
        ...
        $cli->run('lvsctl reconfigure' . ' 1>&null');
        $cli->run('lvsctl stop' . ' 1>&null');
        $cli->run('lvsctl start' . ' 1>&null');
