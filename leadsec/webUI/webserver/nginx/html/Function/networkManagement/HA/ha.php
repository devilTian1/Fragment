<?php
    ...
    define('STATFP', '/etc/ha.d/all_status');
    ...
    function freshStatus() {
        $result = array();
        $curDevName = '';
        $line = fileLinesToArr(STATFP, '', 1, 'includeFunc');
    ...
    function getHAStatus() {
        ...
        $sql = 'SELECT ha_status FROM system';
        $result   = $db->query($sql)->getFirstData(PDO::FETCH_ASSOC);
        $haStatus = $result['ha_status'];
        $errMsg = '';
        if ($haStatus === '1' && !file_exists(STATFP)) {
            $errMsg = '读取双机状态信息出错!';
        } else if ($haStatus === '0') {
            $errMsg = '双机服务尚未启动.';
        }
    ...
    if (@$_GET['act']=='getstatus') {
    	...
    	$allstatus = freshStatus();
        if ($errMsg = getHAStatus()) {
            V::getInstance()->assign('errMsg', $errMsg);
        } else {
            // init page data
