<?php
    ...
    if ($filename = $_POST['editId']) {
        ...
        $cmdArr = array("msgctl del filename \"$editname\"",
        	"msgctl add filename \"$filename\"","msgctl restart 1>/dev/null");         	 	      	
        $cli = new cli(); 
    	$cli->setLog("删除名称为".$_POST['editname']."的文件名控制")->run($cmd[0]);
    	$cli->setLog("添加名称为".$_POST['cfilename']."的文件名控制")->run($cmd[1]);
        ...
        $cli = new cli(); 
    	$cli->setLog("添加名称为".$_POST['cfilename']."的文件名控制")->run($cmd[0]);
        ...
        $cmdArr = array("msgctl del filename \"$filename\"",
        	"msgctl restart 1>/dev/null");        	      	
        $cli = new cli(); 
    	$cli->setLog("删除名称为".$_POST['delId']."的文件名控制")->run($cmd[0]);
        $cli->setLog("重新启动消息传输服务")->run($cmd[1]); 
