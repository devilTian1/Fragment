<?php
    ...
    function checkBannedContent($name){
    	$db  = new dbsqlite(DB_PATH . '/netgap_fs.db');
    	$sql = "SELECT * FROM filter WHERE blacklist = '".$name."'";
    ...
    if ($id = $_POST['editId']) {
        ...
        $rc = checkBannedContent($name);
        if($rc != 0){
        	echo json_encode(array('msg' => "此内容黑名单策略被引用，不能被修改。"));
        }else{
        ...
        $rc = checkBannedContent($_POST['delName']);
        if($rc != 0){
