<?php
    ...
    function checkMailAddr($delname){
    	$db  = new dbsqlite(DB_PATH . '/netgap_mail.db');
    	$sql = "SELECT * FROM filter WHERE allowsender = '".$delname."' OR allowrecv = '".$delname."'";
    ...
    if (!empty($_POST['editId'])) {
    	...
    	$cli  = new cli();
    	$name = $_POST['mailName'];
    	$rc = checkMailAddr($name);
    	if($rc != 0){
    		echo json_encode(array('msg' => "此邮件地址策略被引用，不能被修改。"));
    	}else{    		
    	...
    	$rc = checkMailAddr($_POST['delName']);
    	if($rc != 0){
