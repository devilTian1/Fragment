<?php
    ...
    // Clean Up
        $cli = new cli();
        $cmd = "interfacemon -c 1>/dev/null";
        $cli->setLog("清除数据文件")->run($cmd);
        $cli->setLog("刷新网络状态")->run("getics");
    ...
    // fresh Charts
        ...
        echo json_encode(array('msg'=>$_POST['externalName'],'inline' => $in,'outline' => $out));
        ...
        $cardStatus = '0';
        $cli = new cli();
        $ret = $cli->setLog("刷新网络状态")->setGetResult(true)->run("getics");
        if (empty($ret[0])) {
            $cardStatus = '1';
        }
