<?php
    ...
    if ($id = $_POST['editId']) {
        ...
        $cli = new cli();
        $cli->setLog("修改定制UDP普通访问服务端任务,任务号为".$_POST['customUdpId'])->run(getCmd());
        echo json_encode(array('msg' => '修改成功。'));
    } else if ('add' === $_POST['type']) {
        // Add a new udp general server data
        $cli = new cli();
        $cli->setLog("添加定制UDP普通访问服务端任务,任务号为".intval($_POST['customUdpId']))->run(getCmd());
