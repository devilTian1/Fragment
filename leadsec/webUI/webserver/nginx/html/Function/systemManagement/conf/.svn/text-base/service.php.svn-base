<?php
    ...
    if ($action_ctcp = $_POST['action_ctcp']) {
        ...
        if ($action_sync === 'disable') {
            ...
            $cli = new cli();
            $cli->setLog("文件交换服务关闭")->run($cmd);
            echo json_encode(array('status' => 0, 'msg' => "[$name_sync]已关闭."));
        } else if ($action_sync === 'enable') {
            $cmd = "fsctl start 1>/dev/null"; 
            $cli = new cli();
            $cli->setLog("文件交换服务启动")->run($cmd);
        ...
        if ($action_fsnew === 'disable') {
            ...
            $cli = new cli();
            $cli->setLog("文件同步服务关闭")->run($cmd);
            echo json_encode(array('status' => 0, 'msg' => "[$name_fsnew]已关闭."));
        } else if ($action_fsnew === 'enable') {
            $cmd = "fs2ctl start 1>/dev/null"; 
            $cli = new cli();
            $cli->setLog("文件同步服务启动")->run($cmd);
        ...
        if ($action_http === 'disable') {
           	$cmd = "httpctl stop";         
            $cli = new cli();
            $cli->setLog("安全浏览服务关闭")->run($cmd);
            echo json_encode(array('status' => 0, 'msg' => "[$name_http]已关闭."));
        } else if ($action_http === 'enable') {
            $cmd = "httpctl_start.sh 1>/dev/null"; 
            $cli = new cli();
            $cli->setLog("安全浏览服务启动")->run($cmd);
        ...
        // init service
        $ser_name = array(ctcp_status,    cudp_status, db_status,
                          db_swap_status, sync_status, fs_new_status,
                          http_status,    sync_msg_status);
        $list_out = array();
        //从数据库netgap_system.db中读取服务
        $count = count($ser_name);
        for ($i=0; $i < $count; $i++) {
            if (!getLicense($ser_name[$i])) {
                continue;
            }
        ...
        $list_out_mail = array(); 
        $count = count($mail_name);
        for ($i=0; $i < $count; $i++) {
            if (!getLicense($mail_name[$i])) {
                continue;
            }
        ...
        $list_out_ftp = array();
        $count = count($ftp_name);
        for ($i = 0; $i < $count; $i++) {
            if (!getLicense($ftp_name[$i])) {
                continue;
            }
        ...
        $fastpass_name = array(status);
        $list_out_fastpass = array(); 
        $count = count($fastpass_name);
        for ($i = 0; $i < $count; $i++) {
            if (!getLicense($fastpass_name[$i])) {
                continue;
            }
            $db  = new dbsqlite(DB_PATH . '/netgap_fastpass.db');
            $sql = "SELECT ".$fastpass_name[0]." FROM fastpass_status";
            $result_fastpass = $db->query($sql)
                  ->getFirstData(PDO::FETCH_ASSOC);
            //取得服务的状态值 ：开启为 1 、停止为 0  
            $value = $result_fastpass[$fastpass_name[0]];
            $list_name = array();
            $list_name['name'] = '安全通道';
            $list_name['value']= $value;                              
