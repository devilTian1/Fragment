...
<table class="column_95">
  <tbody>
    <tr>
      ...
      <td class="tdbody">
      	<input name="active" id="active" type="button" value="<{if $active eq 'on'}>停止<{elseif $active eq 'off'}>开始<{/if}>" class="button" onClick="resActive()"/>
        <input name="clean" type="button" value="清空数据文件" class="button" onClick="openDelRes()"/>
       </td>
    </tr>
  </tbody>
</table>
