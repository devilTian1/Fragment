<form action="Function/client/msgTrans/localUserAuthen.php" method="POST" id="editLocalUsrAuthenForm" onSubmit="return false;">
	<input type="hidden" name="type" value="<{$type|default: 'add'}>"/>
	<fieldset>
    	<legend>本地用户认证</legend>
    	<div class="row">
    		<label>用户名:<em class="required">*</em></label>
    		<input class="id" type="text" name="userAuthenName" value="<{$editUsrMng.username}>"
    		<{if $type === 'edit'}> disabled="disabled" <{/if}> size="15" maxlength="15"/>
    		<{if $type ==='edit'}>
            	<input type="hidden" name="userAuthenName" value="<{$editUsrMng.username}>"/>
            <{/if}>
    	</div>
...
$(document).ready(function() {	
	validateForm($("#editLocalUsrAuthenForm"));	
});
</script>