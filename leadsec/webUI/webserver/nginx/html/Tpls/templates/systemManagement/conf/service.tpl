    ...
    <tr>
	    ...
	    <tr>
	      ...
	      <td>
			...
			<img class="policy" id="img_<{$service.index}>" src="<{$smarty.const.THEME_PATH}>/images/icon/select.png"
				width="16" height="16" />
			<input id="action_<{$service.index}>" type="hidden" name="action_<{$service.index}>" value="disable"/></a>
	        <{else}>
	      		<a href="#" onClick="switchService('<{$service.name}>', 'enable','<{$service.index}>')">
	        	<img class="policy" id="img_<{$service.index}>" src="<{$smarty.const.THEME_PATH}>/images/icon/stop.png"
				width="16" height="16"/>
	                <input id="action_<{$service.index}>" type="hidden" name="action_<{$service.index}>" value="enable"/></a>
	        <{/if}>
	        </form>
	        </td>       
	    </tr>
	<{foreachelse}>
	    <tr><td colspan='2'>无服务</td></tr>
	<{/foreach}>
    </tr>
   </tbody>
</table>
