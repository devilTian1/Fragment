...
/**
 * zDialog 2.0 20100427
 * 王朝辉
 * QQ：4845587 E-mail:wzh@wangzhaohui.com
 * 最后修正：2010-5-11
 **/
var scripts = document.getElementsByTagName('script');//lj
var a = scripts[scripts.length - 1];
s=a.src 
s=s.substr(0,s.lastIndexOf('/'))
if(s)s=s+'/';
//alert(s)
var IMAGESPATH = s+'images/'; //图片路径配置
...
Dialog.prototype.create = function () {
	var vw=0;var vh=0; if (!this.Width)vw=1; if (!this.Height)vh=1; //lj
    var bd = $bodyDimensions(topWin);
    ...
    var html = '\
	<style type="text/css">.zbutton{height:25px;width:70px; color:#000;background:url(@IMAGESPATH@bt.gif); border:none;}</style><table id="_DialogTable_@thisID@" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: -moz-none; font-size:12px; line-height:1.4;border-collapse: collapse;">\
	...
	' + (function (obj) {
        ...
        if (obj.URL) return '<iframe width="100%" height="100%" frameborder="0" style="position0:absolute;border:none 0;" id="_DialogFrame_' + obj.ID + '" src="' + obj.displacePath() + '"></iframe><div name="lj" class="loadBox" id="_DialogLoad_' + obj.ID + '" style="position: absolute;left:50%; top:50%;margin:-50px 0 0 -100px; width:200px; height:100px; background:url(@IMAGESPATH@loading.gif) no-repeat center;background-color:#fff; opacity: 0.8;filter:alpha(opacity=80);padding:10px; text-align:center;display:none;">正在加载...</div>';
        return "";
    })(this) + '\
              </div></td>\
          </tr>\
          <tr id="_ButtonRow_@thisID@" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_@thisID@" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input class="zbutton" style="background:url(@IMAGESPATH@bt.gif) no-repeat ; width:70px; height:25px; border:none;" type="button" value="' + this.OkButtonText + '" id="_ButtonOK_@thisID@"/>\
                <input class="zbutton" style="background:url(@IMAGESPATH@bt.gif) no-repeat ; width:70px; height:25px; border:none;" type="button" value="' + this.CancelButtonText + '" onclick="Dialog.getInstance(\'@thisID@\').close();" id="_ButtonCancel_@thisID@"/>\
              </div></td>\
          </tr>\
        </table></td>\
...
//			_DialogLoad_' + obj.ID'
            try {
				...
				//lj
				this.tipClose();
				//topWin.$id("_DialogLoad_" + self.ID).style.display='none';//lj 修改 2010-8-31
				//if(vh>0)this.Height=self.innerDoc.body.offsetHeight+60;
				//if(vw>0) this.Width=self.innerDoc.body.offsetWidth;
				if(vh>0)this.Height=self.innerDoc.body.scrollHeight+60;
				if(vw>0) this.Width=self.innerDoc.body.scrollWidth+40;
				if(this.Height>bd.clientHeight-60)this.Height=bd.clientHeight-60
				if(this.Width>bd.clientWidth-60)this.Width=bd.clientWidth-60
				self.setSize(this.Width,this.Height);
				//alert(self.innerDoc.body.offsetHeight)
				//lj
...
Dialog.prototype.tipShow = function (s) {
s=s||'正在处理数据...';
topWin.$id("_DialogLoad_" + this.ID).style.display='';//lj
topWin.$id("_DialogLoad_" + this.ID).innerHTML=s;
}

Dialog.prototype.tipClose = function () {
topWin.$id("_DialogLoad_" + this.ID).style.display='none';//lj
}
...
Dialog.prototype.show = function () {
	...
	if(this.URL>'')this.tipShow('正在加载页面...')
    ...
    this.opened = true;
	bgdiv=null;
	//fadeEffect(bgdiv,0,100,ielt7?20:10);
...
Dialog.prototype.addButton = function (id, txt, func) {
    ...
    button.type = "button";
    button.style.cssText = "margin-right:5px;background:url(@IMAGESPATH@bt.gif) no-repeat ; width:70px; height:25px;";
    button.value = txt;
    button.onclick = func;
